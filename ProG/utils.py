...
def load_data4pretrain(dataname='CiteSeer', num_parts=200):
    data = pk.load(open(os.path.join(os.path.dirname(__file__), '../dataset/{}/feature_reduced.data'.format(dataname)), 'br'))
    print(data)
    ...
    # hid_dim = input_dim
    graph_list = list(ClusterData(data=data, num_parts=num_parts, save_dir=os.path.join(os.path.dirname(__file__), '../dataset/{}/').format(dataname)))

    return graph_list, input_dim

# used in pre_train.py
def load_data4pretrain_bigscale(dataname='Reddit', per_class_num=10, num_class=41):
    
    graph_list = []
    for class_id in range(num_class):
        
        data_path = os.path.join(os.path.dirname(__file__), '../dataset/{}/induced_graphs/task{}.meta.train.support'.format(dataname, class_id))
    
        with open(data_path, 'br') as t1s:
            t1s = pk.load(t1s)
            t1s = t1s['pos'][:per_class_num]
