...
class Prompted_GNN(torch.nn.Module):
    def __init__(self, input_dim, hid_dim=None, out_dim=None, gcn_layer_num=2, pool=None, gnn_type='GAT', token_num=10, cross_prune=0.1, inner_prune=0.3):
        super().__init__()
        ...
        self.input_project = torch.nn.Linear(input_dim, hid_dim)
        ...
        if gcn_layer_num < 2:
            ...
            self.conv_layers = torch.nn.ModuleList([GraphConv(hid_dim, hid_dim), GraphConv(hid_dim, out_dim)])
            self.pg_layers = torch.nn.ModuleList([HeavyPrompt(token_dim=hid_dim, token_num=token_num, cross_prune=cross_prune, inner_prune=inner_prune),
                                                  HeavyPrompt(token_dim=hid_dim, token_num=token_num, cross_prune=cross_prune, inner_prune=inner_prune)])
        else:
            layers = [GraphConv(hid_dim, hid_dim)]
            pg_layers = [HeavyPrompt(token_dim=hid_dim, token_num=token_num, cross_prune=cross_prune, inner_prune=inner_prune)]
            
            for i in range(gcn_layer_num - 2):
                layers.append(GraphConv(hid_dim, hid_dim))
                pg_layers.append(HeavyPrompt(token_dim=hid_dim, token_num=token_num, cross_prune=cross_prune, inner_prune=inner_prune))
                
            layers.append(GraphConv(hid_dim, out_dim))
            pg_layers.append(HeavyPrompt(token_dim=hid_dim, token_num=token_num, cross_prune=cross_prune, inner_prune=inner_prune))
            
            self.conv_layers = torch.nn.ModuleList(layers)
            self.pg_layers = torch.nn.ModuleList(pg_layers)
    ...
    def forward(self, graph_batch: Batch):
        graph_batch.x = self.input_project(graph_batch.x)
        ori_x, ori_edge_index, ori_batch = graph_batch.x, graph_batch.edge_index , graph_batch.batch
        
        for conv, pg, bn in zip(self.conv_layers[0:-1], self.pg_layers[0:-1], self.bns):
            prompted_graph_batch = pg(graph_batch)
            x, edge_index, batch = prompted_graph_batch.x, prompted_graph_batch.edge_index , prompted_graph_batch.batch
            ...
            graph_batch.x = x[prompted_graph_batch.prompt_mask]

        prompted_graph_batch = self.pg_layers[-1](graph_batch)
        x, edge_index, batch = prompted_graph_batch.x, prompted_graph_batch.edge_index , prompted_graph_batch.batch
        node_emb = self.conv_layers[-1](x, edge_index)
        
        node_emb = node_emb[prompted_graph_batch.prompt_mask] + ori_x
        
        batch = batch[prompted_graph_batch.prompt_mask]
    ...
    def frozen_gnn(self):
        ...
        for pg in self.pg_layers:
            for param in pg.parameters():
                param.requires_grad = True
    
    def get_pg(self):
        return self.pg_layers


class Prompted_GNN_Only_Pre(torch.nn.Module):
    def __init__(self, input_dim, hid_dim=None, out_dim=None, gcn_layer_num=2, pool=None, gnn_type='GAT', token_num=10, cross_prune=0.1, inner_prune=0.3):
        super().__init__()
        ...
        self.pg_layer = HeavyPrompt(token_dim=hid_dim, token_num=token_num, cross_prune=cross_prune, inner_prune=inner_prune)
        self.bns = torch.nn.ModuleList([torch.nn.BatchNorm1d(hid_dim) for i in range(gcn_layer_num-1)])
    ...
    def forward(self, graph_batch: Batch):
        ...
        prompted_graph_batch = self.pg_layer(graph_batch)
        x, edge_index, batch = prompted_graph_batch.x, prompted_graph_batch.edge_index , prompted_graph_batch.batch
    ...
    def frozen_gnn(self):
        ...
        for param in self.pg_layer.parameters():
            param.requires_grad = True
...
class HeavyPrompt(LightPrompt):
    ...
    def forward(self, graph_batch: Batch):
        ...
        for g in Batch.to_data_list(graph_batch):
            ...
            prompt_mask = torch.ones(x.shape[0], dtype=torch.bool)
            prompt_mask[:token_num] = 0

            data = Data(x=x, edge_index=edge_index, y=y, prompt_mask=prompt_mask)
...
class FrontAndHead_ori(torch.nn.Module):
...
class FrontAndHead(torch.nn.Module):
    def __init__(self, hid_dim=16, num_classes=2,
    ...
    def forward(self, graph_batch):
        graph_emb = self.gnn(graph_batch)
        pre = self.answering(graph_emb)
