...
def acc_f1_over_batches(args, test_loader, gnn, answering, num_class, task_type):
    if task_type == "multi_class_classification":
        ...
        mc_auroc = torchmetrics.classification.AUROC(task="multiclass", num_classes=num_class)
    ...
    for batch_id, test_batch in enumerate(test_loader):
        ...
        ma_auc = mc_auroc(pre, y)
        
        print("Batch {} Acc: {:.4f} | Macro-F1: {:.4f} | Macro-AUROC: {:.4f}".format(batch_id, acc.item(), ma_f1.item(), ma_auc.item()))

    acc = accuracy.compute()
    ma_f1 = macro_f1.compute()
    ma_auc = mc_auroc.compute()
    print("Final True Acc: {:.4f} | Macro-F1: {:.4f} | Macro-AUROC: {:.4f}".format(acc.item(), ma_f1.item(), ma_auc.item()))
    accuracy.reset()
    macro_f1.reset()
    mc_auroc.reset()
