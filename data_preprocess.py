...
from torch_geometric.datasets import Planetoid, Amazon, Reddit, DBLP, CitationFull
...
if __name__ == '__main__':
    dataname = 'dblp'  # 'CiteSeer'  # 'PubMed' 'Cora' 'Computers'  'Reddit'   'ogbn-arxiv'  'ogbn-products' 'dblp'
    #
    ...
    if dataname in ['CiteSeer', 'PubMed', 'Cora']:
        ...
        dataset = CitationFull(root='./dataset', name=dataname, pre_transform=pre_transform)
        data = dataset.data
    ...
    for process in ['node', 'edge', 'graph']:
