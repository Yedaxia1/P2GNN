...
def meta_test_adam(meta_test_task_id_list,
    ...
    for task_1, task_2, support, query, _ in load_tasks('test', task_pairs, dataname, K_shot, seed):
        ...
        np.save("./loss_data/{}.npy".format('supervised'),np.array(loss_list))
...
def model_components(args):
    ...
    gnn = BaseGNN(input_dim=args['input_dim'], hid_dim=100, out_dim=100, 
                  gcn_layer_num=2, pool=None, gnn_type=args['gnn_type'])
    
    model = FrontAndHead(hid_dim=100, num_classes=2,  # 0 or 1
    ...
    return maml, gnn, opt, None, lossfn
