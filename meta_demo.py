...
from ProG.prompt import GNN, FrontAndHead, Prompted_GNN, Prompted_GNN_Only_Pre

def get_parameter_number(models):
    res = {'Total': 0, 'Trainable': 0}
    for model in models:
        total_num = sum(p.numel() for p in model.parameters())
        trainable_num = sum(p.numel() for p in model.parameters() if p.requires_grad)
        
        res['Total'] = res['Total'] + total_num
        res['Trainable'] = res['Trainable'] + trainable_num
    ...
    for task_1, task_2, support, query, _ in load_tasks('test', task_pairs, dataname, K_shot, seed):
        ...
        for _ in range(adapt_steps_meta_test):
            ...
            for batch_id, support_batch in enumerate(support_loader):
                ...
                #     running_loss = 0.
            print("epoch: {}, loss: {}".format(_, running_loss/len(support_loader)))
            loss_list.append(running_loss/len(support_loader))
        
        print(get_parameter_number([test_model]))
        np.save("./loss_data/{}.npy".format('Ours'),np.array(loss_list))
        # torch.save(test_model.state_dict(),
        #                    "./Transferability/{}/{}.{}.{}.pth".format(domain_transfer_task_level, dataname, pre_train_method, gnn_type))
...
def model_components(args):
    ...
    # gnn = Prompted_GNN_Only_Pre(input_dim=args['input_dim'], hid_dim=args['hid_dim'], out_dim=args['out_dim'], gcn_layer_num=args['gcn_layer_num'], pool=None,
    #                     gnn_type=args['gnn_type'], token_num=args['prompt_token_num'], cross_prune=args['cross_prune'], inner_prune=args['inner_prune'])
...
if __name__ == '__main__':    
    ...
    domain_transfer_task_level = 'node_level'     # 'node_level' 'graph_level'  'edge_level'
    
    if domain_transfer_task_level == 'graph_level':
        meta_task_id_list = list(range(2 * num_class, 3 * num_class))
    elif domain_transfer_task_level == 'edge_level':
        meta_task_id_list = list(range(num_class, 2 * num_class))
    elif domain_transfer_task_level == 'node_level':
